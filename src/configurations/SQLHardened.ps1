<#
    ...
    .EXAMPLE
        PS C:\> SQLHardened -ComputerName 'localhost' -OutputPath "C:\Temp" -Verbose
    ...
    .NOTES
        ...
        Important: This DSC configuration script also makes use of a PSD data file for full configuration flexibility. The accompanying SQLHardened.psd1 file contains default/example values for some items and should be updated before using this configuration.
...
Configuration SQLHardened {
    ...
    Node $AllNodes.NodeName  {
        ...
        SqlScript 'SQL Baseline - DBA Team Operator Schedule' {
        ...
        SqlScript 'SQL Baseline - Configure SQL Agent Job History' {
            ServerInstance = $Node.Nodename
        ...
        #* TDE Databases job
        SqlScript 'SQL Baseline - Configure TDE Databases' {
            ServerInstance = $Node.Nodename
        ...
        SqlScript 'SQL Baseline - Configure Remote Query Timeout' {
            ServerInstance = $Node.Nodename
        ...
        SqlScript 'SQL Baseline - Configure Remote Access' {
            ServerInstance = $Node.Nodename
        ...
        #* Cycle Error Log Job
        SqlScript 'SQL Hardening - Connection Limit trigger' {
            ServerInstance = $Node.Nodename
            GetFilePath    = "$($ConfigurationData.NonNodeData.ScriptPath)\CycleErrorLogGetScript.sql"
            TestFilePath   = "$($ConfigurationData.NonNodeData.ScriptPath)\CycleErrorLogTestScript.sql"
            SetFilePath    = "$($ConfigurationData.NonNodeData.ScriptPath)\CycleErrorLogSetScript.sql"
            QueryTimeout   = 30
        }

        #* Configure Get/Set/Test Queries: Create Server Roles
        SqlScript 'SQL Baseline - Check if dbconnectrole exists' {
            ServerInstance = $Node.Nodename
        ...
        SqlScript 'SQL Baseline - Grant required permissions to dbconnectrole' {
            ServerInstance = $Node.Nodename
        ...
        #* Configure Get/Set/Test Queries: Create Server Roles
        SqlScript 'SQL Baseline - Check if publicrole exists' {
            ServerInstance = $Node.Nodename
        ...
        #* Configure Get/Set/Test Queries:Server Role permissions
        SqlScript 'SQL Baseline - Grant required permissions to publicrole' {
            ServerInstance = $Node.Nodename
        ...
        #* Revoke execute permissions on stored procedures for "public"
        SqlScript 'SQL Hardening - Revoke Public Permissions - STIG V-41251' {
            ServerInstance = $Node.Nodename
            GetFilePath    = "$($ConfigurationData.NonNodeData.ScriptPath)\STIGRevokePublicGetScript.sql"
            TestFilePath   = "$($ConfigurationData.NonNodeData.ScriptPath)\STIGRevokePublicTestScript.sql"
            SetFilePath    = "$($ConfigurationData.NonNodeData.ScriptPath)\STIGRevokePublicSetScript.sql"
            QueryTimeout   = 30
        }

        #* DISA_STIG_AUDIT
        SqlScript 'SQL Hardening - Auditing' {
            ServerInstance = $Node.Nodename
            GetFilePath    = "$($ConfigurationData.NonNodeData.ScriptPath)\STIGAuditGetScript.sql"
            TestFilePath   = "$($ConfigurationData.NonNodeData.ScriptPath)\STIGAuditTestScript.sql"
            SetFilePath    = "$($ConfigurationData.NonNodeData.ScriptPath)\STIGAuditSetScript.sql"
            QueryTimeout   = 30
        }
    }
}

Write-Host "##[command] Compiling SQLHardened configuration..."
SQLHardened -ConfigurationData "$($Destination)\..\SQLHardened.psd1" -OutputPath "$($Destination)" -Verbose
Rename-Item -Path "$($Destination)\localhost.mof" -NewName "SQLHardened.mof" -Verbose
Write-Host "##[command] Creating SQLHardened checksum..."
New-DscChecksum -Path "$($Destination)\SQLHardened.mof" -Force -Verbose