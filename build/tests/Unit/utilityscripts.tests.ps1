...
Write-Host "##[debug] Scripts Path: $($script:SourceScripts)"

if (Get-Module -Name $script:ProjectName -ErrorAction "SilentlyContinue") {
    Remove-Module -Name $script:ProjectName -Force
}

$script:scripts = Get-ChildItem -Path $script:SourceScripts -Filter "*.ps1"
Write-Host "##[debug] Scripts: $($script:scripts)"

Describe -Name "##[section] Testing Utility Scripts for Module $($script:ProjectName)" {

    Context -Name "##[command] Running PSScriptAnalyzer Tests on Utility Scripts" {
...
Describe -Name "##[section] Testing Help Content" {
    foreach ($script in $script:scripts) {
        Write-Host "##[debug] $($script)"
        $scriptName = $script
        $scriptPath = "$($script:SourceScripts)\$($script)"
